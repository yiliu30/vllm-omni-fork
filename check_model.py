...
model_dir = (
    sys.argv[1]
    if len(sys.argv) > 1
    else "/root/.cache/modelscope/hub/models/asdazd/Qwen3-Omni-30B-A3B-Instruct_modelopt_FP8"
)
...
total_gb = (
    sum(os.path.getsize(os.path.join(model_dir, s)) for s in shards if os.path.exists(os.path.join(model_dir, s))) / 1e9
)
