...
def _tag() -> str:
    return os.environ.get(_DUMP_TAG_ENV, "").strip() or os.path.basename(
        os.environ.get(_DUMP_DIR_ENV, "").rstrip("/")
    ) or "dump"
    ...
    def subsample(self, n: int, device: torch.device) -> torch.Tensor:
        ...
        if idx is None:
            ...
            if tokens <= 0 or n <= tokens:
                ...
                idx = (
                    torch.randperm(n, generator=gen)[:tokens]
                    .sort()
                    .values
                    .to(device)
                )
...
def _diff_metrics_1d(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    diff = a - b
    return torch.stack(
        [dot / (na * nb + _EPS), diff.norm() / (nb + _EPS), diff.abs().max(), diff.abs().mean()]
    )


def _per_head_diff(
    a_h: torch.Tensor, b_h: torch.Tensor
) -> torch.Tensor:
...
def record_linear(prefix: str, x2d: torch.Tensor, y2d: torch.Tensor, layer) -> None:
    ...
    try:
        ...
        xs_f = xs.float()
        in_stats = torch.stack(
            [xs_f.square().mean().sqrt(), xs_f.abs().max()]
        ).cpu().numpy()
    ...
    try:
        ...
        if kind == "self" and out_hnd is not None and is_self:
            ...
            ref = F.scaled_dot_product_attention(
                q_hnd[:, :, idx].contiguous(), k_hnd, v_hnd, scale=softmax_scale
            )
            ...
            k_rms = _head_rms(k_hnd, None).cpu().numpy()
            out_rms = (
                out_hnd.float().square().mean().sqrt().cpu().numpy()
                if out_hnd is not None
                else np.float32(0.0)
            )
            _RECORDER._append(
                _RECORDER.attn, (expert, block, kind), p, (None, q_rms, k_rms, out_rms)
            )
    except Exception as error:  # debug feature must never break the run
...
def save() -> None:
    ...
    for (expert, block, op), rows in sorted(
        ...
        if not complete:
            continue
        payload[f"lin_{expert}_{block}_{op}_diff"] = _pad_pass_array(
            rows, complete[0][0].shape, 0
        )
        payload[f"lin_{expert}_{block}_{op}_in"] = _pad_pass_array(
            rows, complete[0][1].shape, 1
        )
        payload[f"lin_{expert}_{block}_{op}_inch"] = _pad_pass_array(
            rows, complete[0][2].shape, 2
        )
    for (expert, block, kind), rows in sorted(
        _RECORDER.attn.items(), key=lambda kv: (kv[0][0], kv[0][1], kv[0][2])
    ):
        while len(rows) < _RECORDER.pass_count:
            rows.append(None)
        complete = [r for r in rows if r is not None]
